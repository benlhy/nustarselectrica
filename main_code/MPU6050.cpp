...
/**
 * @param frq the number of times per second update() will be called
 */
MPU6050::MPU6050(int frq) {
    frequency = frq;
}
 ...
 */
void MPU6050::update(double rad_vel[]) {
    Wire.beginTransmission(ADDRESS);
    ...
    for (int i = 0; i < 6; i++) {
        ...
        rad_vel[i] *= DEGCONV;
        gyro_pos[i] += rad_vel[i] / frequency;
    }
    Wire.endTransmission(true); //release i2c
}

/**
 * Identical to update(double[]), but does not require passing a variable.
 */
void MPU6050::update() {
    double x[3];
    MPU6050::update(x);
